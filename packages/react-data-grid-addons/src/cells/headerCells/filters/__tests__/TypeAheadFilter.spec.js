...
function fakeGetValidFilterValuesForTypeAhead(column, searchText) {
    const options = [];
    for (let i = 1; i <= 2; i++) {
        options.push({ label: 'Title ' + i, value: i });
    }
    updatedColumnId = column.key;
    updatedSearchText = searchText;
    const response = { then: function() {
        return options;
    } };
    return response;
}

describe('TypeAheadFilter', () => {
    let component;
    let rows;
    ...
    describe('Basic tests', () => {
        ...
        beforeEach(() => {
            component = TestUtils.renderIntoDocument(
                <TypeAheadFilter
                    ...
                    column={fakeColumn}
                    getValidFilterValuesForTypeAhead={fakeGetValidFilterValuesForTypeAhead}
                />);
            rows = createRows();
        });

        it('should create a new TypeAheadFilter instance', () => {
            expect(component).toBeDefined();
        });

        it('should hanlde change', () => {
            const value = [{ value: '1' }];
        ...
        describe('When input value changes', () => {
            it('Should call to get matching dropdown values', () => {
                component.getOptions('abcde', 'click');
                expect(updatedSearchText).toEqual('abcde');
                expect(updatedColumnId).toEqual('title');
            });
        });
    });
});
