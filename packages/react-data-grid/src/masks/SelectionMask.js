...
const isFrozenColumn = (columns, { idx }) =>
  columnUtils.isFrozen(columnUtils.getColumn(columns, idx));

const isScrollingHorizontallyWithoutCellChange = ({
  scrollTop,
  prevScrollTop,
  scrollLeft,
  prevScrollLeft,
...
function SelectionMask({ children, ...rest }) {
  ...
  const position =
    frozen && isScrollingHorizontallyWithoutCellChange(rest)
      ? "fixed"
      : "absolute";
  return (
    <CellMask {...dimensions} className="rdg-selected" position={position}>
      {children}
    </CellMask>
  );
}
