...
function Cell({ column, colSpan, isCellSelected, isDraggedOver, row: row$1, rowIdx, className, onMouseDown, onCellMouseDown, onClick, onCellClick, onDoubleClick, onCellDoubleClick, onContextMenu, onCellContextMenu, onRowChange, selectCell, style, rangeSelectionMode,...props }) {
	...
	*/
	...
	function handleKeyDown(event) {
		...
		if (event.shiftKey) switch (event.key) {
			case "ArrowUp":
				if (selectedRange.endRowIdx > 0) setSelectedRange({
					...selectedRange,
			...
			case "ArrowRight":
				...
				break;
			case "ArrowLeft":
				if (selectedRange.endColumnIdx > 0) setSelectedRange({
					...selectedRange,
					endColumnIdx: selectedRange.endColumnIdx - 1
				});
				break;
	...
	function handleCellPaste(event) {
		...
		if (enableRangeSelection) {
			...
			if (!onCellPaste || !isCellEditable(selectedPosition)) return;
			const { idx, rowIdx } = selectedPosition;
