...
interface IRowRendererProps<R, SR = never> extends SharedCanvasProps<R, SR> {
  rowIdx: number;
  row: R;
...
function RowRenderer<R, SR = never>({
  columnMetrics,
  viewportColumns,
  eventBus,
  rowIdx,
...
export default memo(RowRenderer) as <R, SR = never>(props: IRowRendererProps<R, SR>) => JSX.Element;
