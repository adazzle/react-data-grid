...
interface CalculatedColumnsArgs<R, SR, FR>
  extends Pick<DataGridProps<R, SR, FR>, 'defaultColumnOptions'> {
...
export function useCalculatedColumns<R, SR, FR>({
  rawColumns,
  columnWidths,
  viewportWidth,
  scrollLeft,
  ...
  const { columns, colSpanColumns, lastFrozenColumnIndex, groupBy } = useMemo(() => {
    ...
    const colSpanColumns: CalculatedColumn<R, SR, FR>[] = [];
...
function clampColumnWidth<R, SR, FR>(
  width: number,
  { minWidth, maxWidth }: Column<R, SR, FR>,
