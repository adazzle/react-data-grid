...
describe('InteractionMasks', () => {
  function setup(overrideProps?: Partial<InteractionMasksProps<Row, never>>, initialPosition?: Position) {
    const onSelectedCellChange = jest.fn();
    const props: InteractionMasksProps<Row, never> = {
      height: 100,
      columns,
  ...
  describe('Keyboard navigation functionality', () => {
    ...
    describe('using keyboard to navigate through the grid by pressing Tab or Shift+Tab', () => {
      ...
      const tabCell = (props: Partial<InteractionMasksProps<Row, never>>, shiftKey?: boolean, state?: { selectedPosition: Position }) => {
      ...
      const assertExitGridOnTab = (props: Partial<InteractionMasksProps<Row, never>>, shiftKey?: boolean, state?: { selectedPosition: Position }) => {
        const { wrapper, preventDefaultSpy } = tabCell(props, shiftKey, state);
        expect(preventDefaultSpy).not.toHaveBeenCalled();
        assertGridWasExited(wrapper);
      };

      const assertSelectedCellOnTab = (props: Partial<InteractionMasksProps<Row, never>>, shiftKey?: boolean, state?: { selectedPosition: Position }) => {
