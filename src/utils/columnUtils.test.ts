...
describe('canEdit', () => {
  ...
  it('should return correct booleans', () => {
    ...
    expect(canEdit({ ...column, oldEditor }, row)).toBe(true);
    expect(canEdit({ ...column, oldEditor, editable: false }, row)).toBe(true);
    expect(canEdit({ ...column, oldEditor, editable: true }, row)).toBe(true);
  });
});
