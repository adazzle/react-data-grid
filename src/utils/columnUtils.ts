...
export function getColumnMetrics<R, SR>(metrics: Metrics<R, SR>): ColumnMetrics<R, SR> {
  ...
  const columns: Array<Column<R, keyof R, SR> & { width: number | void }> = [];

  for (const metricsColumn of metrics.columns) {
    const width = getSpecifiedWidth<R, SR>(metricsColumn, metrics.columnWidths, metrics.viewportWidth, metrics.minColumnWidth);
  ...
  const calculatedColumns: CalculatedColumn<R, keyof R, SR>[] = columns.map((column, idx) => {
...
export function canEdit<R, SR>(column: CalculatedColumn<R, keyof R, SR>, row: R): boolean {
...
export function getColumnScrollPosition<R, SR>(columns: readonly CalculatedColumn<R, keyof R, SR>[], idx: number, currentScrollLeft: number, currentClientWidth: number): number {
  let left = 0;
  let frozen = 0;
