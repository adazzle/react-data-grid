...
- **Removed:**
  ...
  - **Props:**
    ...
    - ⚠️ `onCellCopyPaste`
    - ⚠️ `rowsContainer`
    - ⚠️ `onRowDoubleClick`, `onRowClick`, `onGridKeyDown`, `onGridKeyUp`
      - Use standard `div` props on the `Row` component itself instead, via `rowRenderer`:
      ```jsx
      function RowRenderer(props) {
        return <Row onClick={(event) => {}} {...props} />;
      }

      return <DataGrid {...props} rowRenderer={RowRenderer}>;
      ```
